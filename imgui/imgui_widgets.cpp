...
// taken from my old loader
namespace colors {

    const ImVec4 SecondColor = { 0.431372549f, 0.117647059, 0.666666667f, 0.8f };
...
void ImGui::HoverItemText(const char* text, const char* fmt) {
...
bool ImGui::CustomButton(const char* label, const char* text, float sizeX, float sizeY)
{
    ...
    window->DrawList->AddRectFilled(MIN, MAX, ImGui::GetColorU32(it_anim->second.ButtonColor), 10.0f, 0);
...
bool ImGui::CustomCheckbox(const char* label, bool* v)
{
    ...
    it_anim->second.check_rect = ImLerp(it_anim->second.check_rect, *v ? 1.0f : (hovered ? 0.0f : 0.0f), 0.08f * (1.0f - ImGui::GetIO().DeltaTime));

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(32, 33, 35), 3.0f);
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(110 / 255.0f, 30 / 255.0f, 170 / 255.0f, it_anim->second.selected_rect), 3.0f);
    window->DrawList->AddShadowRect(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(110 / 255.0f, 30 / 255.0f, 170 / 255.0f, it_anim->second.selected_rect), 15, ImVec2(0, 0), 0, 3.f);

    RenderCheckMark(window->DrawList, ImVec2(total_bb.Min.x + 5, (total_bb.Min.y + total_bb.Max.y) / 2 - 5), ImColor(0.0f, 0.0f, 0.0f, it_anim->second.check_rect), 10.0f);
...
bool ImGui::CustomBeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(1.0f, 1.0f, 1.0f, 0.02f), 3.0f);
...
// Old API, prefer using BeginCombo() nowadays if you can.
bool ImGui::CustomCombo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!CustomBeginCombo(label, preview_value, ImGuiComboFlags_None))
        return false;
...
bool ImGui::CustomSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 slider_size(w - 8, label_size.y + 16);
    const ImVec2 frame_size(w - 12, label_size.y + 16);

    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + slider_size);
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 10), total_bb.Max);
    const ImRect total_bb2(window->DC.CursorPos, window->DC.CursorPos + frame_size);
    ...
    auto& anim_data = anim[id];  // Direct access or creation if not found
    anim_data.opacity = ImLerp(anim_data.opacity, IsItemActive() ? 1.0f : 0.4f, 0.08f * (1.0f - g.IO.DeltaTime));
    ...
    static float slider_anim = 10.0f;
    slider_anim = ImLerp(slider_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 12.0f);

    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x - 12, frame_bb.Max.y), ImColor(15, 15, 15), 0.0f);
    // window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + slider_anim, grab_bb.Max.y + 2), ImColor(60, 120, 255), 5.0f); // Same Color
    window->DrawList->AddShadowRect(frame_bb.Min, ImVec2(frame_bb.Min.x + slider_anim, frame_bb.Max.y), ImColor(110, 30, 170), 20, ImVec2(0, 0), 0, 3.f);
    window->DrawList->AddRectFilledMultiColor(frame_bb.Min, ImVec2(frame_bb.Min.x + slider_anim, grab_bb.Max.y + 2), ImColor(80, 10, 130), ImColor(110, 30, 170), ImColor(110, 30, 170), ImColor(80, 10, 130)); // Multicolor

    RenderText(total_bb.Min, label);

    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, anim_data.opacity));
    {
        RenderTextClipped(total_bb2.Min, total_bb2.Max, value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.0f));
    }
    PopStyleColor();
...
// also taken from an old loader of mine
...
bool ImGui::CustomInputText(const char* label, const char* hint, float posX, float posY, float sizeX, static char buf[], size_t buf_size, ImGuiInputTextFlags flag)
{
    ImGuiStyle& style = ImGui::GetStyle();
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGuiIO& io = ImGui::GetIO();
    ...
    if (it_anim == anim.end()) {
        ...
        it_anim->second.ShadowColor = ImColor(110, 30, 170);
    }
    ...
    window->DrawList->AddRectFilled({ pos.x + MIN.x, pos.y + MAX.y }, { pos.x + MAX.x, pos.y + MAX.y }, ImColor(110, 30, 170), 1.f);
    window->DrawList->AddLine({ pos.x + MIN.x, pos.y + MAX.y }, { pos.x + MAX.x, pos.y + MAX.y }, ImColor(110, 30, 170), 1.f);
    ...
    ImVec4 ShadowTarget = ImGui::IsItemActive() ? ImColor(110, 30, 170) : ImColor(110, 30, 170);
...
bool ImGui::CustomTab(const char* name, float posX, float posY, bool boolean)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect iconpos(pos, ImVec2(pos.x + posX, pos.y + posY));
    ...
    static std::map <ImGuiID, TabElement> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end()) {
    ...
    it_anim->second.element_opacity = ImLerp(it_anim->second.element_opacity, (hovered ? 0.04f : boolean ? 0.04f : 0.0f), 0.09f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(1.0f, 1.0f, 1.0f, it_anim->second.element_opacity), 15.0f);

    window->DrawList->AddText(NULL, 22.0f, ImVec2(iconpos.Min.x + posX,(iconpos.Min.y + iconpos.Max.y) / 2 - label_size.y / 2), ImColor(1.0f, 1.0f, 1.0f, it_anim->second.text_opacity), name);

    window->DrawList->AddRectFilled(ImVec2(rect.Min.x - 11, rect.Min.y + 11), ImVec2(rect.Min.x - 8, rect.Max.y - 11), ImColor(110 / 255.0f, 30 / 255.0f, 170 / 255.0f, it_anim->second.rect_opacity), 10.0f, ImDrawFlags_RoundCornersRight);
    window->DrawList->AddShadowRect(ImVec2(rect.Min.x - 11, rect.Min.y + 11), ImVec2(rect.Min.x - 8, rect.Max.y - 11), ImColor(110 / 255.0f, 30 / 255.0f, 170 / 255.0f, it_anim->second.rect_opacity), 10, ImVec2(0, 0), 0, 3.f);
...
bool ImGui::SubTab(bool selected, const char* label, const ImVec2& size_arg)
{
    ...
    it_anim->second.bg_op = ImLerp(it_anim->second.bg_op, selected ? 0.8f : (hovered ? 0.5f : 0.0f), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImColor(255, 255, 255) : hovered ? ImColor(255, 255, 255, 200) : ImColor(255, 255, 255, 150), g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(110 / 255.f, 30 / 255.f, 170 / 255.f, it_anim->second.bg_op), 4.f);
    GetWindowDrawList()->AddText(bb.Min + ImVec2(size_arg.x - 42 - CalcTextSize(label).x / 2, (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
